...
impl BlockFetcher {
    ...
    pub async fn fetch_range(
        ...
        start_slot: u64,
        end_slot: u64,
    ) -> Vec<(u64, Result<FetchedBlock>)> {
        let mut results = Vec::new();
        
        for slot in start_slot..=end_slot {
            let result = self.fetch_block(slot).await;
            results.push((slot, result));
            
            // logging every 100 blocks
            if (slot - start_slot) % 100 == 0 && slot != start_slot {
                info!(
                    "Progress: {}/{} blocks fetched",
                    slot - start_slot + 1,
    ...
    pub async fn fetch_range_parallel(
        &self,
        start_slot: u64,
        end_slot: u64,
        concurrency: usize,
        ...
        info!(
            "fetching block range {} to {} with concurrency {}",
